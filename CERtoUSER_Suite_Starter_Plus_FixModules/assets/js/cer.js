...
let energyCerSelect;
let energyPeriodInput;
let energyFeedback;
let energyPeriodLabel;
let energyProductionPeriod;
let energyConsumiWrap;
let energyConsumiTableBody;
let energyConsumiEmpty;
let energyProductionWrap;
let energyProductionTableBody;
let energyProductionEmpty;
...
const ENERGY_ALLOWED_ROLES = new Set(['prosumer', 'produttore', 'producer', 'consumer']);
...
function updateCerRecord(cerId, updater) {
  ...
  const draft = {
    ...
    consumi: (current.consumi || []).map(entry => ({ ...entry })),
    produzione: (current.produzione || []).map(entry => ({ ...entry }))
  };
...
function normalizeCerData(cer) {
  ...
  const produzione = Array.isArray(cer.produzione)
    ? cer.produzione
        .map(entry => {
          const plantId = String(entry.plantId || entry.plant_id || '').trim();
          const period = String(entry.period || '').trim();
          const kwhValue = entry.kwh ?? entry.kwh_totali ?? entry.kwh_total ?? entry.value;
          const kwh = Number(kwhValue);
          if (!plantId || !period) return null;
          const plant = plantMap.get(plantId);
          if (!plant || !isProductionAllowedRole(plant.titolareRuolo)) return null;
          if (!Number.isFinite(kwh) || kwh < 0) return null;
          return {
            id: entry.id || uid('cer-produzione'),
            plantId,
            period,
