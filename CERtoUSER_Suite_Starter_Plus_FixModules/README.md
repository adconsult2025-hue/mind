...
1. **Configura il client** — la configurazione non è più hardcoded nel repository. Il file `config/firebase-config.js` tenta prima di leggere `window.__FIREBASE_RUNTIME_CONFIG__`, poi esegue una chiamata sincrona a `/.netlify/functions/firebase-config` che restituisce le variabili d'ambiente esposte lato server. La funzione accetta sia le nuove chiavi dedicate (`FIREBASE_WEB_API_KEY`, `FIREBASE_AUTH_DOMAIN`, `FIREBASE_DATABASE_URL`, `FIREBASE_PROJECT_ID`, `FIREBASE_STORAGE_BUCKET`, `FIREBASE_MESSAGING_SENDER_ID`, `FIREBASE_APP_ID`, `FIREBASE_MEASUREMENT_ID`) sia le varianti già in uso nei deploy esistenti (`FIREBASE_API_KEY`, `REACT_APP_FIREBASE_*`, `NEXT_PUBLIC_FIREBASE_*`) o un blob JSON (`FIREBASE_CONFIG_JSON`). Assicurati quindi di valorizzare almeno una di queste chiavi nell'ambiente Netlify (o nel tuo `.env` locale) e, opzionalmente, di fornire `window.__FIREBASE_RUNTIME_CONFIG__` durante lo sviluppo offline.
