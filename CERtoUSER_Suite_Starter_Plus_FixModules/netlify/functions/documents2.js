...
const err=(s,m)=>({statusCode:Number.isFinite(+s)?+s:500,headers:{'Content-Type':'application/json'},body:JSON.stringify({ok:false,error:{code:String(s),message:m}})});

async function buildContext(refType, refId){
  if (refType === "CER") {
    return {
      CER:{ Nome:"CER Ponte Grande", FormaGiuridica:"Associazione", SedeLegale:"Via Roma 1",
  ...
  return {
    CER:{ Nome:"CER Ponte Grande", CodiceFiscale:"12345678901",
         CabinaPrimaria:{Codice:"CP-001", Descrizione:"Ponte Grande"} },
...
exports.handler = async (event) => {
  try {
    ...
    try {
      // cerca per slug o id (niente "code")
      const t = await client.query("SELECT id,type FROM templates WHERE slug=$1 OR id::text=$1 LIMIT 1", [templateSlug]);
      ...
      const safeSlug = String(templateSlug || templateId).trim() || `template-${templateId}`;
