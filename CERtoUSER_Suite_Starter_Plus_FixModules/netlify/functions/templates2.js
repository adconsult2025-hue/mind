      ...
      try {
        ...
        try {
          const result = await client.query(`
            SELECT t.id, t.name, t.slug, t.code, t.type, t.module,
                   (SELECT MAX(version) FROM template_versions v WHERE v.template_id = t.id) AS latest_version
            FROM templates t
            ORDER BY t.created_at DESC`);
          rows = result.rows;
        } catch (queryErr) {
          if (queryErr?.code !== '42703') throw queryErr; // colonne non esistenti
          const fallback = await client.query(`
            SELECT t.id, t.name, t.slug, t.type,
                   (SELECT MAX(version) FROM template_versions v WHERE v.template_id = t.id) AS latest_version
            FROM templates t
            ORDER BY t.created_at DESC`);
          rows = fallback.rows.map(r => ({ ...r, code: null, module: null }));
        }
        ...
        const safe = rows.map((r) => {
          ...
          const slug = firstNonEmpty(r.slug, r.code, r.name, r.id);
          const code = firstNonEmpty(r.code, r.slug, r.name, r.id);
          const module = firstNonEmpty(r.module, 'CER');
          const version = r.version ?? r.latest_version ?? null;
